// EnemyControllerVolador.cs
using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class EnemyControllerVolador : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            ...
            if (playerScript != null && !playerScript.EstaRecibiendoDanio())
            {
                float dirX = collision.transform.position.x - transform.position.x;
                Vector2 direccionDanio = new Vector2(Mathf.Sign(dirX), 0);
